...
# import wave # For writing recognized keywords as WAV files
...
class RobotStateMachine:
    def __init__(self):
        ...
        self.transitions = {
            "stopped": ["go", "left", "right", "down"],
            "moving": ["stop"],
            "down": ["up"],
        }

    def valid_commands(self):
        # Return the valid commands based on the current state
    ...
    def execute_command(self, command):
        ...
        if command in self.valid_commands():
            ...
            print(f"Invalid command '{command}' for current state '{self.state}'")

    def update_state(self, command):
        # Update the state machine based on the command
        if command == "go":
            self.state = "moving"
        elif command == "stop":
            self.state = "stopped"
            ...
            self.state = "stopped"  # Return to stopped state after "up"
...
# commands = ["go", "stop", "left", "right", "down", "up", "go"]
        ...
        try:
            while True:
                ...
                if is_keyword:
                    ...
                    # wf = wave.open( f"{kw}_{kw_prob * 100:.1f}%.wav", 'wb')
                    # wf.setnchannels(1)      # Mono
                    # wf.setsampwidth(2)      # 16 bit samples are 2 bytes each
